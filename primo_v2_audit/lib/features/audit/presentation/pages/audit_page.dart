...
class _AuditPageContent extends StatelessWidget {
  ...
  void _changeSort(BuildContext context, AuditSortField field) {
    final currentState = context.read<AuditBloc>().state;
    final currentSort = currentState is AuditLoaded
        ? currentState.currentSort
        : AuditSort.defaultSort;

    // Toggle order if same field
    final newOrder = currentSort.field == field &&
            currentSort.order == AuditSortOrder.descending
        ...
        subtitle: Column(
          ...
          children: [
            if (event.employeeId != null)
              Text('Empleado: ${event.employeeId}'),
            if (event.metadata != null)
              Text('Detalles: ${event.metadata}'),
            Text(
